...
async function fetchCustomers() {
    try {
        ...
        for (const id in data.data) {
            const cust = data.data[id];
            const div = document.createElement('div');
            div.className = 'customer';
            div.innerHTML = `
                <strong>ID: ${cust.customer_ID}</strong><br>
                Name: ${cust.name}<br>
                Phone: ${cust.phone_number}<br>
                Email: ${cust.email}<br>
                Address: ${cust.address}<br>
                <button onclick="updateCustomer(${cust.customer_ID})">Update</button>
                <button onclick="deleteCustomer(${cust.customer_ID})">Delete</button>
            `;
...
document.getElementById('addCustomerForm').addEventListener('submit', async function(e) {
    ...
    const rawPhone = customer.phone_number.replace(/\D/g, ''); // Remove non-digits
...
document.getElementById('updateCustomerForm').addEventListener('submit', async function(e) {
    ...
    if (!name || !email || !address) {
        showModal('errorModal', 'Name, email, and address are required', false);
        return;
    }
